  ...
  $getLOG = isset($_GET["log"]) && $_GET["log"] !=null ? $_GET["log"] :"";
    ...
    if ($_SERVER["REQUEST_METHOD"]=="POST") {
        ...
        if (isset($_POST["LOGForm"])) {
          echo "LOGForm";
          $user = $_POST["logName"];
          $pass = FCalss::HashPass($_POST["logPass"]);

          $NamerowCount = my_FCalss::cHECKexist("ID","users","WHERE Name LIKE ?","",$user);
          $PassCheck    = my_FCalss::cHECKexist("ID","users","WHERE Name LIKE ?","AND Pass LIKE ?",$user,$pass);
          $CheckData    = FCalss::getOne("*","users","WHERE Name LIKE ?","AND Pass LIKE '%$pass%'",$user);
          if($PassCheck){
            echo "good pass";
          }else{
            echo "bad pass";
